...
By default, `moment#diff` will return the number rounded towards zero (down for positive, up for negative). If you want the floating point number, pass `true` as the third argument. Before **2.0.0**, `moment#diff` returned rounded number, not a rounded *towards zero* number.
